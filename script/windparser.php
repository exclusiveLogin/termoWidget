...
if($get_windinterval && $get_windmin && $get_windmax ){
    $res = $mysql->query("SELECT DATE_FORMAT(`datetime`,'%Y,%m,%d,%H,%i,%S') AS `datetime`,`value` FROM `log_wind` WHERE UNIX_TIMESTAMP(`datetime`) BETWEEN $get_windmin AND $get_windmax ORDER BY `datetime` ASC");
}
else{
...
//echo "]";
$res->close();
$res = $mysql->query("SELECT DATE_FORMAT(`datetime`,'%Y,%m,%d,%H,%i,%S') AS `datetime`,`value` FROM `log_wind` WHERE `datetime`=(SELECT MAX(`datetime`) FROM `log_wind`)");
$row = $res->fetch_assoc();
echo ",";
